function f=f_reward_neural(p,t,OD,TT)
t_e=1;
t_T=10;
N_agent_length=5;%元胞空间边长
kesei=0.01;
kesei_p=0.2;%价格敏感度
theita_l_so=1.5;%交互水平
capacity=100;%拥挤阈值
speed=20;%车速km/小时
%==============线路=============
        ...
        for j=1:num_station
            if transf(i,j)==0%可以直达
                ...
                if i~=j
                    %========选择网约车出行人数=========
                    %%%%%%%%%%%%%%%%%%%%%%%期望效用%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %cell=multi_agent_neural(t_e,i,j,cell,OD,N_agent_length);
                    %%%%%%%%%%%%%%%%%%%%%%%累积前景效用%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    cell=multi_agent_neural_CPT(t_e,i,j,cell,OD,N_agent_length);
...
e_bus=0.036*1000;%新能源公交排放
e_car=0.51*1000;%小汽车排放
for i=1:num_station
    for j=1:num_station
        if i~=j
            bus_profit=[];
            ...
            if transf(i,j)==0
                for k=1:cell{i,j}.route_num
                    ...
                    bus_emission(k)=cell{i,j}.bus_q(k)*e_bus*cell{i,j}.bus_travel_time(k)*speed/60;
                end
                ...
                profit(i,j)=sum(bus_profit(:));%票价收入
